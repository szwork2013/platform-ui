...
const ViewComponent = (props) => {
  ...
  let searchParam = { //搜索条件
    filter: { //过滤规则
      ...
      where: 'o.type=1 and o.isStopProduction=0', //条件
    },
    size: 20, //指定每页记录数
    sort: ['o.year,desc', 'o.name,desc'] //缺省排序规则
  }

  const type = 'OFFICE_EQUIPMENT';//耗材类型编码:it办公设备
  const uploadProps = {
    name: 'excel',
  ...
  const actionBarProps = {
    ...
    newrowPayloadRender: () => {
      return {
        record: {
          _isNew: true, _key: (new Date().getTime() + ''), type: 1,
  ...
  const listProps = {
    columns: [ // 和antd table组件的列定义相同
      {title: '物资名称', width: 195, dataIndex: 'name', key: 'name', sorter: true, editor: <Input/>},
