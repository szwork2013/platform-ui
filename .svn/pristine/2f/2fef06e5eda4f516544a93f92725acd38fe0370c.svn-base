...
const ViewComponent = (props) => {
  ...
  let searchParam = { //搜索条件
    filter: { //过滤规则
      ...
      where: 'o.type=2 and o.isStopProduction=0', //条件
    },
    size: 20, //指定每页记录数
    sort: ['o.year,desc', 'o.name,desc'] //缺省排序规则
  }

  const type = 'NON_INSTALLATION_EQUIPMENT';//耗材类型编码:非安装设备
  const uploadProps = {
    name: 'excel',
    ...
    newrowPayloadRender: () => {
      return {
        record: {
          ...
          type: 2, year: new Date().getFullYear(), isStopProduction: '0', taxRate: 0.17
        }
      }
    },
    buttons
