...
const ViewComponent = ( props ) => {
  ...
  const actionBarProps = {
    ...
    newRow: true,
    newrowPayloadRender:()=>{
      return {record:{_isNew: true,_key: (new Date().getTime()+''),
        org:service.constructRecordUrl({ modelName: 'orgs', id: user.org.id })}}
    }
  }
  ...
  let searchParam={ //搜索条件
    filter: { //过滤规则
      ...
      where: 'o.material.type=6 and o.org.id='+user.org.id,  //条件
    },
    size: 20, //指定每页记录数
  ...
  const selectProps = {
    ...
    searchParam: {
      filter: { //过滤规则
        ...
        where: 'o.type=6 and o.year='+new Date().getFullYear()+' and o.isStopProduction!=1', //条件
      },
      size: 100, //指定每页记录数
