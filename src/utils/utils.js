...
const groupDataList=(list,groupKey,groupRecordFunc)=>{
  ...
  if(!groupKey||!list||list.length==0) return list;

  //无分组就是有一个合计
  if(groupKey=='_undefined'){
    let resultList=[];
    let groupRecord=groupRecordFunc(list,groupKey);
    if(!groupRecord) return list;
    groupRecord.children=[];
    resultList.push(groupRecord);
    list.map((item)=>{
      groupRecord.children.push(item);
      resultList.push(groupRecord);
    });
    //最后返回数据。
    return resultList;
