...
const wfservice = {
  ...
  //设置父流程实例表单连接到记录中
  setParentInstanceEntityLink: (record, parentInstanceEntityLink) => {
    if(!record)
      return ;
    record._parentInstanceEntityLink = parentInstanceEntityLink;
  },

  //获取父流程实例表单连接
  getParentInstanceEntityLink: (record) => {
    if(!record)
      return undefined;
    return record._parentInstanceEntityLink;
  },
...
//启动流程实例
function startProcess({refVarName,processNo,record,parentInstanceId,parentInstanceEntityLink}) {
  ...
  const url = serviceUrl + 'instance?no=' + processNo + '&starterId=' +
    ...
    +'&parentInstanceEntityLink='+parentInstanceEntityLink;
...
function executeAction(todoId,actionId,sendees=[],comment='',record,commentActionId='') {
  ...
  let url = serviceUrl+'instance/todos/'+todoId+'/actions/'+actionId+'/execute?'+
    ...
    +'&commentActionId='+commentActionId+'&entityLink=' + entityLink;
