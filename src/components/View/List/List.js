...
class List extends React.Component{
  render() {
      ...
      //2 构造配置和数据
      //设置视图footer
      this.configFooter(tableProps, model);

      //设置选择配置
      this.configRowSelection(model, rowSelection);
      ...
      //行列计算
      dataSource = this.computeRowAndCol(dataSource, columns);

      //如果树形视图，则转换为树结构数据
      if (dataSource && treedata && !groupKey) {
      ...
      let onRow=undefined;
      this.configDragable(dragAble, components, onRow);
  ...
  //2018-03-06 sgf add  拖动排序 郑波2018-3-12修改：让render变简洁
  configDragable = (dragAble, components, onRow) => {
      if(dragAble) {
        components = {
          body: {
            row: DragRow,
  ...
  //配置视图的footer 郑波2018-3-12修改：让render变简洁
  configFooter = (tableProps, model) => {
      //sgf add 2018-1-29 添加表格底部
      if (tableProps.footer) {
          let footer = tableProps.footer(model.list);
          tableProps = {
              ...tableProps, footer: () => {
                  return footer
              }
  ...
  //设置选择配置：郑波2018-3-12修改：让render变简洁
  configRowSelection = (model, rowSelection) => {
      if (rowSelection) {
  ...
  //郑波2018-3-12修改：让render变简洁
  constructColumns = (columns) => {
      let isTreeColumn = false; //列是否是树结构 zyk 2018-01-24
  ...
  //行列计算 郑波2018-3-12增加
  computeRowAndCol = (dataSource, columns) => {
    if (!dataSource) return dataSource;
    if (dataSource.length == 0) return dataSource;

    //1 列计算
    dataSource = this.computeCol(dataSource, columns);

    //2 行计算
    dataSource = this.computeRow(dataSource, columns);

    return dataSource;
  }

  // 列计算
  computeCol = (dataSource, columns) => {
    if (columns.length == 0) return dataSource;

    //1 获得计算表达式
    let computeFormula = this.getComputeFormula(columns);
    if (computeFormula=="") return dataSource;

    let assignFormula = this.getAssignFormula(columns);
    if (assignFormula=="") return dataSource;

    let formula = assignFormula+";"+computeFormula;

    //2 对每条记录进行计算
    dataSource.forEach((record, index) => {
      this.computeFormula(record, formula);
    })
    return dataSource;
  }

  // 获取所有计算列的计算表达式
  getComputeFormula = (columns) => {
    let formula = "";
    columns.forEach((col, index) => {
      let colFormula = "";
      if (col.children) { //包含子列
        colFormula = this.getComputeFormula(col.children);
      }
      else { //不包含子列
        colFormula = this.getComputeFormulaOfCol(col);
      }
      if (colFormula!="")
        formula += (formula==""?"":";")+colFormula;
    })
  ...
  // 获取指定列的计算表达式
  getComputeFormulaOfCol = (col) => {
    let attrName = col.dataIndex;
    if (!attrName||attrName=="") return "";
    if (col._formula) { 
      return "record['"+attrName+"']="+col._formula;
    }
    else {
      return "";
    }
  ...
  // 获取所有列的赋值表达式
  getAssignFormula = (columns) => {
    let formula = "";
    columns.forEach((col, index) => {
      let colFormula = "";
      if (col.children) { //包含子列
        colFormula = this.getAssignFormula(col.children);
      }
      else { //不包含子列
        colFormula = this.getAssignFormulaOfCol(col);
      }
      if (colFormula!="")
        formula += (formula==""?"":";")+colFormula;
    })
  ...
  // 获取指定列的赋值表达式
  getAssignFormulaOfCol = (col) => {
    let attrName = col.dataIndex;
    if (!attrName||attrName=="") return "";
    if (!col._formula) { //非计算列，构造赋值表达式
      return "let "+attrName+"=record['"+attrName+"']||0";
    } 
    else { //计算列，无需构造赋值表达式
      return "";
    }
  ...
  // 计算表达式
  computeFormula = (record, formula) => {
    if (formula=="") return;
    eval(formula);
  }

  // 行计算
  computeRow = (dataSource, columns) => {
    return dataSource;
  }
  //---END---
