...
class ViewComponent extends React.Component {
  ...
  render() {
    ...
    let searchParam = { //搜索条件
      filter: { //过滤规则
        ...
        where: 'o.submitDate!=null and o.status<2', //条件
      },
      size: 20, //指定每页记录数
    ...
    const listProps = {
      columns: [ // 和antd table组件的列定义相同
        ...
        {
          title: '状态', width: 150, dataIndex: 'status', key: 'status', sorter: true, fulltext: false,
  ...
  statusSearchProps=(dispatch)=> {
    const statusProps = {
      ...
      initialValue: this.value ? this.value : '2',
      onChange: value => {
        this.value=value;
        const searchParam = {
          filter: { //过滤规则
            clazz: 'Draft', //模型对应的后台实体类
            where: value == 2 ? 'o.submitDate!=null and o.status<2' : ('o.submitDate!=null and o.status=' + value), //条件
          },
          size: 100, //指定每页记录数
