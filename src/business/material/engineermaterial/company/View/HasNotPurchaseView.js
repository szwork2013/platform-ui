...
const ViewComponent = (props) => {
  let buttons = [];
  buttons.push({ title: '生成采购计划', OnClick: () =>{
    props.dispatch({
      type:'demand_plan/createPurchase',
      payload:{type:4,orgLevel:0}
    })
  }});
  //定义操作条属性
  ...
  let searchParam = { //搜索条件
    filter: { //过滤规则
      ...
      jpql:'SELECT o FROM DemandPlan o where o.type=4 and o not in (SELECT DISTINCT d FROM PurchasePlan p join p.demandPlans d where d.type=4)',
      //jpql:'SELECT DISTINCT o FROM PurchasePlan p join p.demandPlans d RIGHT JOIN DemandPlan o ON o!=d where o.type=4',
      where: '', //条件
    },
    size: 20, //指定每页记录数
