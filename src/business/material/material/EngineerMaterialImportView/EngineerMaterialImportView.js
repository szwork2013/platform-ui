...
const ViewComponent = (props) => {
  ...
  const type = 'ENGINEERING_MATERIALS';//耗材类型编码:工程物资
  const uploadProps = {
    name: 'excel',
    ...
    newrowPayloadRender: () => {
      return {
        record: {
          ...
          type: 4, year: new Date().getFullYear(), isStopProduction: '0', taxRate: 0.17
        }
      }
    },
    buttons,
  ...
  const listProps = {
    columns: [ // 和antd table组件的列定义相同
      {title: '物资名称', width: 160, dataIndex: 'name', key: 'name', sorter: true, editor: <Input/>},
      ...
      {title: '税率', width: 100, dataIndex: 'taxRate', key: 'taxRate', sorter: true, editor: <Input/>},
      {title: '转换关系（吨/米）', width: 195, dataIndex: 'convertRule', key: 'convertRule', sorter: true, editor: <Input/>},
