...
const modelParam = {
  ...
  effects: {
    ...
    *inventoryIn ({payload}, { call, put,select }) {
      ...
      let { record } = payload;
      const user = service.userInfo.user;
      let result=yield call(service.updateRecord,service.parseRecordUrl(record)+'/inventory_in?orgId='+user.org.id,{});
      if(result){
        message.info('入库成功');
        yield put({
          type: 'purchase_plan/refreshTodo',
          payload
        });
      }
    },
    ...
    //设置批次号
    *initNo({ payload }, { call, put, select }) {
      let { record, } = payload;
      let result = yield call(service.queryRecord,
        config.API + '/purchase_plan/get_init_no', {});
      if (result);
      record.serialNo = result;
    },
