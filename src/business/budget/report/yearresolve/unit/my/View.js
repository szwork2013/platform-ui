...
class MyDeptView extends React.Component{
  ...
  render(){
    ...
    //获取当前用户部门id
      ...
      let searchParam={ //搜索条件
        ...
        filter: { //过滤规则
          ...
          where: 'o.reportType=2 and o.periodType=0 and o.orgLevel=0 and o.createdBy.org.id='+deptId, //条件
        },
        size: 20, //指定每页记录数
      ...
      const actionBarProps = {
        new: true, //显示新增按钮
        newPayloadRender: () => {
            let record={reportType:0,periodType:0,orgLevel:0};
            let origin = lodash.cloneDeep(record);
            let model = {record, state:{mode:'new', origin}};
            return {processNo:'YEARDEMANDCOMPANY',model}
          },
        delete: true, //显示删除按钮
        newRow:false
      }
    ...
    return (
        <View {...props}
          uid='yearresolveunit_my'
          key='yearresolveunit_my'
