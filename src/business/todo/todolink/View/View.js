...
const ViewComponent = (props) => {
  ...
  const {dispatch, todo_link: {viewData}} = props;
  ...
  const buttons = <Button type="primary" disabled={!(selectedRowKeys && selectedRowKeys.length > 0)}
                          onClick={handleButtonClicked}>移入已办</Button>
  ...
  const searchParam = {
    projection: 'default',
    filter: { //过滤规则
      clazz: 'TodoLink', //模型对应的后台实体类
      where: `o.sendeeId='${user.id}'`, //条件
    },
    size: 20, //指定每页记录数
    sort: ['o.sentTime,desc','o.state,asc'] //未读升序,紧急程度降序,创建时间降序
  };
...
export default connect(({todo_link, loading}) =>
  ({todo_link, loading: loading.models[modelName]})
)(ViewComponent);
