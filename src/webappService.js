...
const service = {
	...
	//批量保存记录
	batchSaveRecord: async(url, saveData) => {
		return batchSaveRecord(url, saveData);
	},
...
function parseRestResult(data) {
  ...
  //sgf 2018-01-08 add 后台返回的是数组 直接构造数据返回
...
//郑波2018-1-24增加：批量保存记录
function batchSaveRecord(url, saveData) {
	let param = {
    method: 'post',
    data: saveData
	}
